...
名字 | 描述
----|-----
`catalog`      |     [操作应用商店](#rancher-catalog-reference)
`config`     |       [设置客户端配置](#rancher-config-reference)
`docker`      |        [在主机上运行docker命令](#rancher-docker-reference)
`environment`, `env`  | [操作环境](#rancher-environment-reference)
`events`, `event`   |        [展示资源变更事件](#rancher-events-reference)
`exec`       |       [在容器上运行命令](#rancher-exec-reference)
`export`     |      [将应用的yml配置文件导出为tar或者本地文件](#rancher-export-reference)
`hosts`, `host`    |   [操作主机](#rancher-hosts-reference)
`logs`           |   [抓取容器的日志](#rancher-logs-reference)
`ps`            |    [展示服务／容器](#rancher-ps-reference)
`restart`       |   [重启服务／容器](#rancher-restart-reference)
`rm`          |      [删除服务、容器、应用、主机、卷](#rancher-rm-reference)
`run`         |     [运行服务](#rancher-run-说明)
`scale`       |      [设置一个服务运行的容器数量](#rancher-scale-reference)
`ssh`         |      [SSH到主机](#rancher-ssh-reference)
`stacks`, `stack`  |   [操作应用](#rancher-stacks-reference)
`start`, `activate`  | [启动服务、容器、主机、应用](#rancher-startactivate-reference)
`stop`, `deactivate` | [停止服务、容器、主机、应用](#rancher-stopdeactivate-reference)
`up`           |     [启动所有服务](#rancher-up-reference)
`volumes`, `volume` |   [操作卷](#rancher-volumes-reference)
`inspect`      |     [查看服务、容器、主机、环境、应用、卷的详情](#rancher-inspect-reference)
`wait`        |      [等待服务、容器、主机、应用栈、机器、项目模版](#rancher-wait-reference)
...
----|-----
`--quiet`, `-q`   |  只展示IDs
`--format` value  | `json`格式或自定义格式: {% raw %}{{.Id}} {{.Name}}{% endraw %}
`--system`, `-s`  |  展示系统模版
...
`--quiet`, `-q`  |  只显示IDs
`--format` value | `json` 或者自定义格式: {% raw %}{{.Id}} {{.Name}}{% endraw %}
...
`rancher env template` 命令用语导出或者导入环境模版。
...
`--env-file` value, `-e` value     |    指定一个包含了环境变变量的文件
...
`--format` value  | `json` 或者自定义格式: {% raw %}{{.Id}} {{.Name}}{% endraw %} (default: "json")
